...
<head>
    ...
    <title>Daftar - TetapKuliah</title>
...
<body class="bg-[#F0F0EB] min-h-screen flex items-center justify-center p-6 md:p-10">
    ...
    <div class="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden relative"
        ...
        <div class="flex flex-col lg:flex-row h-[580px] relative">
            ...
            {{-- LEFT SIDE: FORM --}}
            <div class="flex-1 p-8 sm:p-12 flex flex-col justify-start bg-white overflow-y-auto relative z-10" style="direction: rtl;">
                <div class="w-full max-w-sm mx-auto" style="direction: ltr;">
                ...
                {{-- Heading --}}
                <div class="mb-7">
                    <h1 class="text-2xl font-bold text-gray-900 mb-1.5 tracking-tight">Buat Akun Baru</h1>
                    <p class="text-sm text-gray-400 font-normal">Daftarkan dirimu untuk memulai pendampingan akademik.</p>
                </div>
                ...
                <form action="{{ route('register') }}" method="POST" class="space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <label for="name" class="block text-xs font-semibold text-gray-600 tracking-wide">Nama Lengkap</label>
                        <input
                            id="name"
                            type="text"
                            name="name"
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="space-y-1.5">
                            <label for="semester" class="block text-xs font-semibold text-gray-600 tracking-wide">Semester</label>
                            <input
                                id="semester"
                                type="number"
                                name="semester"
                                ...
                                min="1" max="14"
                                required
                                class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm px-3 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2D4A34]/40 focus:border-[#2D4A34] focus:bg-white text-center font-medium transition-all"
                            >
                        </div>
                        <div class="space-y-1.5">
                            <label for="faculty" class="block text-xs font-semibold text-gray-600 tracking-wide">Fakultas</label>
                            <input
                                id="faculty"
                                type="text"
                                name="faculty"
                                value="{{ old('faculty', 'Teknik') }}"
                                required
                                placeholder="Teknik"
                                class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm px-3 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2D4A34]/40 focus:border-[#2D4A34] focus:bg-white transition-all placeholder-gray-300"
                            >
                        </div>
                        <div class="space-y-1.5">
                            <label for="major" class="block text-xs font-semibold text-gray-600 tracking-wide">Prodi</label>
                            <input
                                id="major"
                                type="text"
                                name="major"
                                value="{{ old('major', 'Informatika') }}"
                                required
                ...
                <p class="mt-6 text-center text-xs text-gray-400">
                    Sudah punya akun?
                    <a href="{{ route('login') }}" data-transition class="text-[#2D4A34] font-bold hover:underline ml-0.5">
                        Masuk Sekarang
                    </a>
                </p>
